...
/**
 * 단일 이미지 업로드 함수 (로그 포함)
 * @param {string} imageFilePath - 업로드할 이미지 파일의 경로
 * @param {string} fileName - 이미지 파일 이름
 * @returns {Promise<string|null>} - 업로드된 이미지의 URL (실패 시 null)
 */
...
/**
 * 폴더 내 모든 이미지 업로드 및 결과 JSON 파일 생성 후,
 * 업로드가 완료된 이미지 파일들을 삭제하는 함수
 */
async function uploadImagesFromFolder() {
  fs.readdir(imagesFolder, async (err, files) => {
    ...
    fs.writeFile(
      ...
      (writeErr) => {
        if (writeErr) {
          console.error("결과 JSON 파일 저장 실패:", writeErr);
        } else {
          console.log(`\n결과 JSON 파일이 저장되었습니다: ${resultFilePath}`);
          // JSON 파일 저장 후, make_img 폴더 내의 이미지 파일들을 삭제
          for (const file of imageFiles) {
            ...
            fs.unlink(filePath, (unlinkErr) => {
              if (unlinkErr) {
                ...
                console.log(`[INFO] ${file} 파일 삭제 완료.`);
              }
            });
          }
        }
