...
async function downloadAndSaveImage(url, filename, name) {
  try {
    ...
    // 액자 효과를 위한 패딩 계산
    const padding = Math.round(Math.min(resizedMetadata.width, resizedMetadata.height) * 0.05);
    ...
    // 텍스트 오버레이를 위한 SVG 생성
    const fontSize = Math.round(Math.min(resizedMetadata.width, resizedMetadata.height) * 0.05);
    const boxWidth = Math.round(resizedMetadata.width * 0.8);
    const boxHeight = Math.round(fontSize * 2);
    const svgOverlay = `
      <svg width="${newWidth}" height="${newHeight}">
        <rect x="${(newWidth - boxWidth) / 2}" y="${(newHeight - boxHeight) / 2}" width="${boxWidth}" height="${boxHeight}" fill="rgba(0,0,0,0.7)" rx="5" ry="5" />
