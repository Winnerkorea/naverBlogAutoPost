...
async function writeResultJson(data) {
  try {
    await fs.writeFile(path.join(__dirname, 'result.json'), JSON.stringify(data, null, 2), 'utf8');
...
async function main() {
  try {
    ...
    const introductionsText = await generateRestaurantIntroduction(restaurantInfos);
    
    const sections = introductionsText.split('<세션');
    
    // hello_content 추출
    const helloContentMatch = sections[1].match(/hello_content:\s*([\s\S]+?)\n\n/);
    const helloContent = helloContentMatch ? removeSpecialCharacters(helloContentMatch[1].trim()) : '';

    const restaurantIntros = sections[2].split('---').map(text => {
    ...
    const content = contentMatch ? removeSpecialCharacters(contentMatch[1].trim()) : '';
